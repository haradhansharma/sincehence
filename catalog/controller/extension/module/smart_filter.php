...
class ControllerExtensionModuleSmartFilter extends Controller {
	...
	}elseif($this->config->get('config_name') == 'sincehence Spoint'){///sharma
	   $results = $this->model_smart_filter_smart_filter->getOptionlod();
		if(!empty($results)){
		foreach ($results as $result) {
			$option_value=$this->model_smart_filter_smart_filter->get_option_value_name($result['option_id']);
			if(!empty($option_value)){
			$data['options'][] = array(
				'option_id'  => $result['option_id'],
				'name'       => $result['name'],
