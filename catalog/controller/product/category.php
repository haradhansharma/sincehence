...
if ($this->config->get('config_store_id') == 0){
    $width = '_image_product_width';
    $height = '_image_product_height';
}else{
    $width = '_image_compare_width';
    $height = '_image_compare_height';
}
///sharma
			foreach ($results as $result) {
	///sharma modified here		    
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], $this->config->get('theme_' . $this->config->get('config_theme') . $width), $this->config->get('theme_' . $this->config->get('config_theme') .  $height));
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', $this->config->get('theme_' . $this->config->get('config_theme') . $width), $this->config->get('theme_' . $this->config->get('config_theme') .  $height));
				}
...
////sharma
			...
			$data['sorts'][] = array(
				'text'  => $this->language->get('Last Order Date'),
				'value' => 'p.slot_date-ASC',
				'href'  => $this->url->link('product/category', 'path=' . $this->request->get['path'] . '&sort=p.slot_date&order=ASC' . $url, true)
			);
			...
			////sharma
		$data['sid'] = $this->config->get('config_store_id');
		////sharma
...
/////sharma
            ...
            $data['cusgroup'] = $this->config->get('config_customer_group_id');
/////sharma            
			$data['sort'] = $sort;
			$data['order'] = $order;
			$data['limit'] = $limit;
