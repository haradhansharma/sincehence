...
///////sharma
   ...
   public function addPreRecord($new_order,$pre_order_id,$pre_slot_date,$pre_paid, $new_customer){
      $this->db->query("CREATE TABLE IF NOT EXISTS `".DB_PREFIX."pre_record` (
	  `id` int(50) NOT NULL AUTO_INCREMENT,
	  `new_order` int(50) NOT NULL,
	  `pre_order` int(50) NOT NULL,
	  `slot_date` int(50) NOT NULL,
	  `pre_paid` int(50) NOT NULL,
	  `new_customer` int(50) NOT NULL,
	  PRIMARY KEY (`id`)
	) ENGINE=MyISAM DEFAULT CHARSET=utf8 AUTO_INCREMENT=1");
	
	$this->db->query("DELETE FROM " . DB_PREFIX . "pre_record WHERE pre_order = '" . (int)$pre_order_id . "'");
	
	$this->db->query("INSERT INTO " . DB_PREFIX . "pre_record SET new_customer = '" . (int)$new_customer . "', new_order = '" . (int)$new_order . "', pre_order = '" . $pre_order_id . "', slot_date = '" . $pre_slot_date . "', `pre_paid` = '" . $pre_paid . "'");
       
   }
   
//   /////sharma to clear order complete
// 	public function cleanincome() {
// 		$this->db->query("DELETE FROM " . DB_PREFIX . "cart_incomplete WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "'");
// 	}
// 	/////sharma to clear order complete

   	public function getPredata(){

    	$query =  $this->db->query("SELECT `customer_id`, `refference_order_id`, `slot_date`, `previous_paid` FROM `" . DB_PREFIX . "cart` WHERE  api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' ");
