...
class ModelCustomerCustomerApproval extends Model {
	...
	/////
	   ...
	   public function checkemail($insert_email) {

		$query=$this->db->query("SELECT email from`" . DB_PREFIX . "spm_store` WHERE email = '" .$insert_email. "'");
		return $query->rows;
	    }

		public function insertdataspm($data) {

		$this->db->query("INSERT INTO  `" . DB_PREFIX . "spm_store` SET email = '" .$this->db->escape($data['email']). "',name = '" .$this->db->escape($data['spmstore']). "', status='0',date_added= NOW()");
		// $spm_id = $this->db->getLastId();
  ...
  //    return $spm_id;
		...
		$query=$this->db->query("SELECT storepickup_id, s.email as spmemail from`" . DB_PREFIX . "spm_store` s LEFT JOIN `".DB_PREFIX."customer` c on (s.email=c.email)");
		return $query->rows;

	    }
		 public function getstorepickupid($get_email) {

		$query=$this->db->query("SELECT storepickup_id,email from`" . DB_PREFIX . "spm_store` where email='".$get_email."'");
		return $query->rows;
		...
		$query=$this->db->query("SELECT customer_group_id from`" . DB_PREFIX . "customer_group_description` where name='Retailer franchise'");
		return $query->rows;

	    }
	    public function getdistributergroupid() {

		$query=$this->db->query("SELECT customer_group_id from`" . DB_PREFIX . "customer_group_description` where name='Distributer franchise'");
		return $query->rows;

	    }
	    public function getallspmemail() {

		$query=$this->db->query("SELECT email from`" . DB_PREFIX . "spm_store` ");
		return $query->rows;
