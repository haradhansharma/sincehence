...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
		    ...
		    $spm_email = '';
            $spm_id = '';
			$res= $this->model_customer_customer_approval->getstorepickupid($result['email']);
            foreach ($res as $spm_res) {  
                 $spm_email = $spm_res['email']; 
                 $spm_id = $spm_res['storepickup_id'];
            }
        ...
        $this->request->server['REQUEST_METHOD'] == 'POST';
        	...
        	$this->request->post['email']=null;
        }

		$insert_email=$this->request->post['email'];
		
        $results=$this->model_customer_customer_approval->checkemail($insert_email);
        foreach ($results as $result) {
        ...
        if($insert_email !=$get_email){ 

		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			$this->model_customer_customer_approval->insertdataspm( $this->request->post);
	    ...
	    $results=$this->model_customer_customer_approval->getretailergroupid();
        foreach ($results as $result) {
        	$data['retailer_group_id']=$result['customer_group_id'];
        }
        $results=$this->model_customer_customer_approval->getdistributergroupid();
        foreach ($results as $result) {
        	$data['distributer_group_id']=$result['customer_group_id'];
        }
        $data['allspmemails'][]=array();
        $results=$this->model_customer_customer_approval->getallspmemail();
        foreach ($results as $result) {
        	$data['allemails'][]=array(
        		'email' => $result['email']);
