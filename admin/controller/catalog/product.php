...
class ControllerCatalogProduct extends Controller {
	...
	public function copytorf() {
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $product_id) {
				$this->model_catalog_product->copyProductrf($product_id);
			}
	...
	protected function getList() {
		...
		////sharma
		$data['copytorf'] = $this->url->link('catalog/product/copytorf', 'user_token=' . $this->session->data['user_token'] . $url, true);
		////sharma
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				/////////
				'enable_add_to_cart'     => $result['enable_add_to_cart'] ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
				////////
	...
	protected function getForm() {
		...
		////////
		if (isset($this->request->post['enable_add_to_cart'])) {
			$data['enable_add_to_cart'] = $this->request->post['enable_add_to_cart'];
		} elseif (!empty($product_info)) {
			$data['enable_add_to_cart'] = $product_info['enable_add_to_cart'];
		} else {
			$data['enable_add_to_cart'] = true;
		}
		/////////
		...
		///////protect from edit spoint franchise product-sharma
		
		$spoint_product_results = $this->model_catalog_product->getSpointProduct();
		...
		if (isset($this->request->post['product_layout'])) {
			...
			//////for safe from edit of autometic order	
		if(isset($this->request->get['product_id'])){
		    $editing_product = $this->request->get['product_id'];
		}else{
		   $editing_product = ''; 
		}	
		$model_info_data = $this->model_catalog_product->getModelp($editing_product);
		if($model_info_data){
		    $data['modelname']=array();
	    	foreach ($model_info_data as $result) {
	    	$data['modelname']=array(
	    	      'model' => $result['model']);
	    	}
		...
		if(empty($model_name)){
	    	...
	    	$model_order = $this->model_catalog_product->getOrdpro($model_name);
	    if($model_order){
		    $data['modelorder']=array();
	    	foreach ($model_order as $result2) {
	    	$data['modelorder']=array(
	    	      'order_id' => $result2['order_id']);
	    	}	
		    $moder_order_id = $result2['order_id'];
		}
		if(empty($moder_order_id)){
		    $moder_order_id = '';
		}
