...
class ControllerSaleOrder extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->get['filter_store_name'])) {
			...
			if (isset($this->request->get['filter_payment_trxID'])) {
			$url .= '&filter_payment_trxID=' . $this->request->get['filter_payment_trxID'];
		}
		/////sharma
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_payment_trxID'])) {
			$filter_payment_trxID = $this->request->get['filter_payment_trxID'];
		} else {
			$filter_payment_trxID = '';
		}
		///////sharma
...
////////////sharma
		...
		$filter_data = array(
			...
			'filter_payment_trxID'    => $filter_payment_trxID,
			//////sharma
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				////sharma
				'payment_trxID' => $result['payment_trxID'],
				'payment_code' => $result['payment_code'],
				'payment_mobile_number' => $result['payment_mobile_number'],
		...
		$data['sort_payment_trxID'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=order_payment_trxID' . $url, true);
		//////sharma
	...
	public function info() {
		...
		if ($order_info) {
            ...
            $refference_id= $data['order_id'];

			$this->load->model('sale/order');

			$data['order_from_retailer']=array();
			$results=$this->model_sale_order->getdatabyorderid($order_id);

			 foreach($results as $result){
			 	$data['order_from_retailer'] = array(
				'spm_shop_name'   => $result['shop_name'],
			    'shop_email'     => $result['shop_email'], 
				'customer_group_id'     => $result['customer_group_id'], 
				'shop_address'     => $result['shop_address'], 
				'shop_city'     => $result['shop_city']);
				$shopemail= $data['order_from_retailer']['shop_email'];
			...
			///// get distributer customer id
			 $data['distributer_customer_id']=array();
			$results=$this->model_sale_order->getdistributer_customer_id($shopemail);
			 ...
			 if(empty($data['distributer_customer_id']['customer_id'])){
			 	$data['distributer_customer_id']['customer_id']=null;
			 }
			 if(empty($data['distributer_customer_id']['customer_group_id'])){
			 	$data['distributer_customer_id']['customer_group_id']=null;
			 }

            $distributer_customer_id=$data['distributer_customer_id']['customer_id'];
            $distributer_customer_group_id=$data['distributer_customer_id']['customer_group_id'];
             ...
             $data['c_credential']=array();

			$results=$this->model_sale_order->getdatabyshopemail($shopemail);
			...
			'customer_group_id'   => $result['customer_group_id'],
			'customer_shop_name'   => $result['shop_name']);
            $customergroupid= $data['c_credential']['customer_group_id'];
		    $customeremail= $data['c_credential']['customer_email'];
		    $customername= $data['c_credential']['customer_name'];
		    $customer_id= $data['c_credential']['customer_id'];
						}
			...
			$customername=null;
             ...
             $data['c_email']=array();
			$results=$this->model_sale_order->getdatabycustomeremail($customeremail);
            ...
            if(empty($data['c_email']['customer_name'])){
            	
            	$data['c_email']['customer_name']=null;
            }
            if(empty($data['c_email']['spm_shop_name'])){
            	
            	$data['c_email']['spm_shop_name']=null;
            }
			
			$emailandname=$data['c_email']['spm_shop_name'] . " of " .$customername;


			$data['refference_order']=array();

			$results=$this->model_sale_order->getdatarefference_order($order_id);
			
			foreach($results as $result){
			$data['refference_order'] = array(
			'refference_order_id'   => $result['refference_order_id']);
		    }
		    if(empty($data['refference_order']['refference_order_id'])){
            	
            $data['refference_order']['refference_order_id']=null;
            }
		    $refference_order_id=$data['refference_order']['refference_order_id'];
            ...
            $this->request->post['comment']=null;
            }
            if(empty($this->request->post['notify'])){
            $this->request->post['notify']=null;
            }
            if(empty($this->request->post['override'])){
            $this->request->post['override']=null;
            }
            $order_statuss_id= $this->request->post['order_status_id'];
            $data['required_comment']= $this->request->post['comment'];
            

            
            $data['groupid']=array();
			$results=$this->model_sale_order->getcustomergroupid();
			
			foreach($results as $result){
			$data['groupid'] = array(

			'con_customer_group_id'   => $result['customer_group_id']);
			$condition_customer_group_id=$data['groupid']['con_customer_group_id'];
		    }
			...
			$data['statusid']=array();
			$results=$this->model_sale_order->getorderstatusid();
			
			foreach($results as $result){
			$data['statusid'] = array(

			'con_order_status_id'   => $result['order_status_id']);
			$condition_order_status_id=$data['statusid']['con_order_status_id'];
            ...
            $results2=$this->model_sale_order->missingactiontakenid();
			
			foreach($results2 as $result){

			$mi_action_t_order_status_id   = $result['order_status_id'];

		    }


		    if($order_statuss_id==$mi_action_t_order_status_id){

            if ($this->request->server['REQUEST_METHOD'] == 'POST')  {
			
			$this->model_sale_order->editorderstatusmissingactiontaken($refference_id);
            ...
            if($order_statuss_id==$condition_order_status_id && $distributer_customer_group_id==$condition_customer_group_id){
            
			if($order_id != $refference_order_id) {
			if ($this->request->server['REQUEST_METHOD'] == 'POST')  {

			$this->model_sale_order->adddatacart( $this->request->post,$emailandname,$refference_id,$distributer_customer_id);
			...
			$data['action'] = $this->url->link('sale/order/info','user_token=' . $this->session->data['user_token'] . '&order_id=' . $this->request->get['order_id'] , true);
	...
	public function invoice() {
		...
		////check spmstore shipping method code sharma start	
			$cusemail = $order_info['email'];
			    $spmdata = array();
				$spmstore_code = $this->model_sale_order->getSpmcode($order_id,$cusemail);
		...
		////check spmstore shipping method code sharma end
...
/////// placed 0 IO '' by sharma
	...
	////replaced else data array for shipping sharma	
			...
			if(empty($spmstore_code['spmmethodcode'])){
	    $spmstore_code['spmmethodcode']=null;
	}
	if( $order_info['shipping_code'] = $spmstore_code['spmmethodcode'] ){
				$data['orders'][] = array(
					'order_id'	       => $order_id,
...
/////sharma	
	public function autocomplete() {
		...
		foreach ($json as $key => $value) {
			$sort_order[$key] = $value['payment_trxID'];
		}
